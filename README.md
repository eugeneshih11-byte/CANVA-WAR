...
Controls: use the menu buttons in the Hub and meta views. In combat, use WASD movement, mouse aiming, hold the primary mouse button to fire, press Q to switch between equipped Weapon Slots A and B, press B for the paused Build Detail view, and click a level-up card or press 1/2/3 to choose its reward. R restarts after Victory, Game Over or Abandon. **Back to Hub** opens the existing paused Abandon confirmation during an active Run; after Death, Victory, or Abandon it returns immediately. Continue or Escape resumes a Run from that confirmation. Logical Wave Coming transitions preserve held movement/firing; modal and terminal boundaries clear them. `?playtest=1` exposes a Hub loadout selector plus deterministic Build and Wave Coming acceptance triggers; ordinary production still starts with Starter alone and exposes neither trigger.
...
The controller phases are `SETTLING`, `NORMAL`, `WAVE_COMING` and `FINAL_COMPLETE`. Enemy lifecycles are `RESERVED`, `ENTERING`, `ACTIVE`, `NEAR_OFFSCREEN`, `RETURNING` and `DEAD`. For the 800 x 600 viewport, Capacity Area is `800 × 600 − π × 100² ≈ 448584.07 px²`; obstacle area is not deducted. Visible Fill is the sum of every living Enemy's visual-rectangle intersection with the viewport divided by Capacity Area. Projected Fill adds committed off-screen spawn and fully off-screen return reservations exactly once. Normal refill uses a 15%/20%/25% hysteresis band and a hard 70% controller commitment ceiling. Wave 1 ramps its effective target from 5% for the first two seconds of actual combat, to 15% until five seconds, then to the normal 20% target. A 120-pixel spatial hash replaces the former all-pairs dynamic-Enemy neighbor scan in the hot movement/placement path.
...
Player/regular-Enemy body separation is independent from attack policy. Regular-Enemy pairs are also separated after ordinary and committed movement with small, shared, terrain-safe corrections drawn only from nearby spatial-hash candidates; large Tank footprints and multi-Enemy compression use their actual collision rectangles. Gunner preserves its 220–320 preferred range and line-of-sight requirement, retreats when crowded too close, navigates toward a clear lane when too far or occluded, and completes its telegraphed two-shot burst once those conditions are recovered. Normal alone retains generic body-contact damage. Fast Strike, Tank Slam, Interceptor Charge, Gunner projectiles, Artillery impacts, Denier hazards, Trapper traps and Tether effects keep their authored mechanisms, while Support has no direct damage.
...
|---|---|
| Starter | Precise baseline projectile: Fire Rate 4, one straight shot |
| Scatter | Five-pellet wide fan for nearby crowd clearing |
| Piercer | Fast narrow projectile with four-enemy Pierce |
...
The Run owns one Build: `sharedUpgrades`, `weaponModsByWeaponId`, `weaponEvolutionByWeaponId`, up to three ranked Passive identities, and discovered hidden Combos. Loadout equipment is not duplicated into Build. Player owns authoritative base Damage 2; legacy per-Weapon Damage calibration is no longer used for production hits. `resolveWeaponStats()` and `resolvePlayerStats()` derive values centrally. Attack Rate percentages share one additive modifier bucket. Split Shot retains its per-projectile factor and is restricted to Starter, Scatter and Piercer.

| Shared Upgrade / Passive | Effect per selection | Cap |
|---|---|---:|
| Rapid Fire | +20% Attack Rate | 4 |
| Heavy Shot | Player Damage +1, Bullet Size +1 where applicable, -8% Attack Rate | 4 |
...
| Swift Feet | +8% movement Speed | 4 |

Arc Blade has the first Weapon-specific Mod, Wide Arc I/II, increasing its 55-degree sweep half-angle to 65/75 degrees. Cyclone Blade becomes a legal normal reward after Wide Arc II and Rapid Fire II; every third Arc Blade attack then sweeps 360 degrees. Cyclone Blade plus Swift Feet II automatically discovers the hidden Blade Dance Combo, changing that cadence to every second attack. Each sweep visits a target at most once and uses normal resolved Player Damage.

Launcher adds the second production Weapon-specific slice. Cluster Shell I/II creates exactly two/three deterministic secondary explosions after the primary impact. Siege Bloom becomes a legal normal reward after Cluster Shell II and shared Heavy Shot II; it commits one blast at the original impact point after 0.45 seconds with 1.5 times the primary radius. Siege Bloom plus Vitality II automatically discovers the hidden Chain Reaction Combo, concentrating the same cluster count from the normal one-primary-radius distribution to 0.6 times that distance. Primary, cluster and bloom events each visit a target at most once and all use resolved Player Damage without a Launcher-specific damage multiplier.

Each level-up displays three legal cards when at least three distinct legal rewards exist. Mouse selection and 1/2/3 choose the matching visible card; Escape cannot dismiss the required choice. Maxed, incompatible, premature Evolution and fourth-new-Passive entries are filtered without semantic synergy weighting. Choice generation uses a seeded Reward RNG isolated from Enemy Type and Position RNG. Build Detail pauses simulation and shows Shared Upgrades, per-equipped-Weapon Mods/Evolution, three Passive slots and discovered Combos; the COMBOS section does not exist before discovery. In `?playtest=1`, the deterministic Build demo follows the equipped authored slice: Launcher presents Cluster Shell I/II, Heavy Shot I/II, Siege Bloom and Vitality I/II as real Level-Up selections, while Arc Blade retains its existing Evolution/Combo demonstration.

Holding the primary mouse button requests attacks from the active slot at its resolved Fire Rate. Cooldowns advance only with active combat simulation, allow at most one newly initiated attack per update and never emit delayed catch-up attacks after a long frame. Level-Up, Enemy Introduction, Build Detail and Abandon pauses freeze them. Wave Coming is non-modal and preserves held movement, firing and cooldown state. Intermission, death, Victory, Abandon, restart and focus loss clear held firing.

An attack is one trigger event and may emit multiple projectiles or one Arc Blade sweep. Split Shot places projectile directions symmetrically around the aim angle. Each projectile carries its resolved Damage, size, speed, Pierce budget, Weapon identity and attack identity. A projectile can hit each regular Enemy or Boss at most once; Pierce permits additional distinct targets before removal, while every Launcher explosion event visits each target once. Committed Siege Bloom effects live in a bounded effect queue independent of their resolved projectile, survive weapon switching and ordinary projectile cleanup, and clear on Run reset or terminal cleanup. Projectile traversal uses identity snapshots and explicitly terminates after synchronous Wave completion, so final-Wave cleanup cannot leave a live loop dereferencing or applying stale projectiles. Playtest telemetry adds bounded Launcher primary/cluster/bloom counts and targets plus Chain Reaction commit counts alongside the existing Build snapshots and reward history.
...
Timings follow active simulation updates, excluding modal pauses. `encounterElapsedTime` and `activeCombatTime` measure observed combat for every Encounter; `actualClearTime` and `clearTimeRatio` are numeric only after a successful clear and remain `null` for Death, Abandon or other unfinished outcomes. `attackEvents` counts accepted trigger events, while `shotsFired` remains the number of projectiles emitted. Per-Weapon metrics distinguish attacks, projectiles, hits, damage, kills, kills per active combat second, Pierce events, explosion targets, Arc Blade targets and Burst shots. Run telemetry captures starting Loadout, every categorized offer/selection, rank before/after, Weapon identity, Evolution and Combo events, Reward RNG state, and final formal Build summary. Boss encounters count actual Charge entries and damaging Charge contacts. Telemetry configuration includes immutable Weapon and Build content, Continuous Encounter calibration and the Fire Rate cap. Telemetry remains observation-only and never consumes gameplay RNG.
