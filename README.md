...
Threat curves/costs, active pressure, Template rhythm/bias, clear-time weights, Boss analysis and Stage scaling are **initial calibration placeholders**, not final balance. Playtest feedback currently reports pressure as too low, but tuning is intentionally deferred until clean reliability/UI data is available. Collect unassisted clear times, damage, composition, Score, attack/projectile efficiency and group-wait duration before tuning. Capacity and Points remain deliberately unrecalibrated. Settlement economics, Stage/Encounter configuration, Threat values, MaxActiveThreat values, Enemy/Boss stats, Score awards, Points, Save schema and Run progression are unchanged. No final performance scoring, Stage 2, Equipment/Item systems, Boss phases/adds, reward content or Run persistence are implemented.
